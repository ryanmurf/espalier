...
async function createPgAdapter(
  ...
  try {
    ...
    throw new Error(
      `Cannot load PostgreSQL adapter. Install "espalier-jdbc-pg" to use adapter "pg".`,
    );
  }
...
async function createMysqlAdapter(
  ...
  try {
    ...
    throw new Error(
      `Cannot load MySQL adapter. Install "espalier-jdbc-mysql" to use adapter "mysql".`,
    );
  }

  const MysqlDataSource = mod.MysqlDataSource as new (config: { mysql: Record<string, unknown> }) => DataSource;
  const MysqlMigrationRunner = mod.MysqlMigrationRunner as new (ds: DataSource, config?: MigrationRunnerConfig) => MigrationRunner;
...
async function createSqliteAdapter(
  ...
  try {
    ...
    throw new Error(
      `Cannot load SQLite adapter. Install "espalier-jdbc-sqlite" to use adapter "sqlite".`,
    );
  }

  const SqliteDataSource = mod.SqliteDataSource as new (config: { filename: string }) => DataSource;
  const SqliteMigrationRunner = mod.SqliteMigrationRunner as new (ds: DataSource, config?: MigrationRunnerConfig) => MigrationRunner;
