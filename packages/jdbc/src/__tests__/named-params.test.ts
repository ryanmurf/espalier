...
describe("parseNamedParams", () => {
  ...
  it("converts multiple named params to positional", () => {
    const result = parseNamedParams("SELECT * FROM users WHERE name = :name AND age > :age");
  ...
  it("reuses the same positional index for duplicate named params", () => {
    const result = parseNamedParams("SELECT * FROM users WHERE name = :name OR alias = :name");
    expect(result.sql).toBe("SELECT * FROM users WHERE name = $1 OR alias = $1");
    expect(result.paramOrder).toEqual(["name"]);
  });

  it("handles mixed duplicate and unique params", () => {
    const result = parseNamedParams("INSERT INTO t (a, b, c) VALUES (:x, :y, :x)");
  ...
  it("handles params with underscores and numbers", () => {
    const result = parseNamedParams("SELECT * FROM t WHERE col_1 = :param_1 AND col2 = :param2");
    expect(result.sql).toBe("SELECT * FROM t WHERE col_1 = $1 AND col2 = $2");
  ...
  it("handles many params in correct order", () => {
    const result = parseNamedParams("UPDATE t SET a = :a, b = :b, c = :c WHERE d = :d");
    expect(result.sql).toBe("UPDATE t SET a = $1, b = $2, c = $3 WHERE d = $4");
