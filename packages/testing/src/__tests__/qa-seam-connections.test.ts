...
import { Table, Column, Id, ManyToOne, createRepository } from "espalier-data";
import {
  withTestTransaction,
...
describe.skipIf(!canConnect)(
  "Seam 1A: withTestTransaction + connection pool release",
  { timeout: 60000 },
  () => {
    ...
    it("nested savepoint: inner data rolled back, outer data preserved", async () => {
      ...
      await withTestTransaction(ds, async (ctx) => {
        const stmt = ctx.connection.createStatement();
        await stmt.executeUpdate(
          `INSERT INTO qa_seam_employees (name, email) VALUES ('OuterEmp', 'outer@nested.test')`,
        );
        ...
        await withNestedTransaction(ctx, async (nCtx) => {
          ...
          const bothCount = await countRows(
            nCtx.connection,
            "qa_seam_employees",
            `name IN ('OuterEmp', 'InnerEmp')`,
          );
        ...
        outerVisible =
          (await countRows(ctx.connection, "qa_seam_employees", `name = 'OuterEmp'`)) === 1;
        innerGoneAfterNested =
          (await countRows(ctx.connection, "qa_seam_employees", `name = 'InnerEmp'`)) === 0;
      });
...
describe.skipIf(!canConnect)(
  "Seam 1B: withQueryLog + existing CrudRepository methods",
  { timeout: 60000 },
  () => {
    let ds: PgDataSource;
    ...
    it("withQueryLog combined with withTestTransaction captures all queries inside tx", async () => {
      ...
      await withTestTransaction(instrumentedDs, async (ctx) => {
        const stmt = ctx.connection.createStatement();
        await stmt.executeUpdate(
          `INSERT INTO qa_seam_employees (name, email) VALUES ('TxLogEmp', 'txlog@seam.test')`,
        );
...
describe("Seam 3: EntityFactory + relationship decorators — unit level", () => {
  ...
  it("factory with unknown trait throws descriptive error", () => {
    const factory = createFactory(SeamEmployee);
    expect(() => factory.build({}, "nonExistentTrait")).toThrow(
  ...
  it("factory.resetSequences resets to initial counter", () => {
    const factory = createFactory(SeamEmployee).sequence(
      "email",
      (n) => `reset${n}@seq.test`,
    );
...
describe.skipIf(!canConnect)(
  "Seam 4: Factory + QueryLog + Transaction triple integration",
  { timeout: 60000 },
  () => {
    let ds: PgDataSource;
    ...
    it("factory sequence numbers do NOT reset between transactions", async () => {
      ...
      const n1 = parseInt(email1.match(/seq(\d+)/)![1]);
      const n2 = parseInt(email2.match(/seq(\d+)/)![1]);
