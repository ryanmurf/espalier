...
import { and, not, or } from "../../query/criteria.js";
...
describe("QueryBuilder.select", () => {
  ...
  it("builds SELECT with entity class resolving columns", () => {
    ...
    expect(q.sql).toBe('SELECT "id", "user_name", "email_address", "age" FROM "users"');
    expect(q.params).toEqual([]);
  });
  ...
  it("builds SELECT with WHERE clause", () => {
    const q = QueryBuilder.select("users").where(col("age").gt(18)).build();
    expect(q.sql).toBe('SELECT * FROM "users" WHERE "age" > $1');
    expect(q.params).toEqual([18]);
  });

  it("builds SELECT with WHERE and AND", () => {
    const q = QueryBuilder.select("users").where(col("age").gt(18)).and(col("user_name").like("J%")).build();
    expect(q.sql).toBe('SELECT * FROM "users" WHERE ("age" > $1 AND "user_name" LIKE $2)');
    expect(q.params).toEqual([18, "J%"]);
  });

  it("builds SELECT with WHERE and OR", () => {
    const q = QueryBuilder.select("users").where(col("status").eq("active")).or(col("status").eq("pending")).build();
    expect(q.sql).toBe('SELECT * FROM "users" WHERE ("status" = $1 OR "status" = $2)');
    expect(q.params).toEqual(["active", "pending"]);
  });

  it("builds SELECT with ORDER BY", () => {
    const q = QueryBuilder.select("users").orderBy("name", "ASC").orderBy("age", "DESC").build();
  ...
  it("builds SELECT with GROUP BY", () => {
    const q = QueryBuilder.select("orders").columns("status", "COUNT(*)").groupBy("status").build();
  ...
  it("builds SELECT with GROUP BY and HAVING", () => {
    const q = QueryBuilder.select("orders")
      ...
      .having(col("cnt").gt(5))
      .build();
    expect(q.sql).toBe('SELECT "status", COUNT(*) as cnt FROM "orders" GROUP BY "status" HAVING "cnt" > $1');
    expect(q.params).toEqual([5]);
  });

  it("builds SELECT with JOIN", () => {
    const q = QueryBuilder.select("users").join("INNER", "orders", "users.id = orders.user_id").build();
    expect(q.sql).toBe('SELECT * FROM "users" INNER JOIN "orders" ON users.id = orders.user_id');
  });

  it("builds SELECT with LEFT JOIN", () => {
    const q = QueryBuilder.select("users").join("LEFT", "profiles", "users.id = profiles.user_id").build();
    expect(q.sql).toBe('SELECT * FROM "users" LEFT JOIN "profiles" ON users.id = profiles.user_id');
  });
  ...
  it("supports compound criteria with and/or/not helpers", () => {
    const criteria = and(col("age").gt(18), or(col("status").eq("active"), not(col("banned").eq(true))));
    const q = QueryBuilder.select("users").where(criteria).build();
    expect(q.sql).toBe('SELECT * FROM "users" WHERE ("age" > $1 AND ("status" = $2 OR NOT ("banned" = $3)))');
  ...
  it("supports BETWEEN criteria", () => {
    const q = QueryBuilder.select("users").where(col("age").between(18, 65)).build();
  ...
  it("supports IS NULL and IS NOT NULL", () => {
    const q = QueryBuilder.select("users").where(col("deleted_at").isNull()).and(col("email").isNotNull()).build();
    expect(q.sql).toBe('SELECT * FROM "users" WHERE ("deleted_at" IS NULL AND "email" IS NOT NULL)');
    expect(q.params).toEqual([]);
  });

  it("and() on empty where sets the criteria", () => {
    const q = QueryBuilder.select("users").and(col("age").gt(18)).build();
    expect(q.sql).toBe('SELECT * FROM "users" WHERE "age" > $1');
    expect(q.params).toEqual([18]);
  });

  it("or() on empty where sets the criteria", () => {
    const q = QueryBuilder.select("users").or(col("age").gt(18)).build();
    expect(q.sql).toBe('SELECT * FROM "users" WHERE "age" > $1');
    expect(q.params).toEqual([18]);
  });
});
...
describe("QueryBuilder.insert", () => {
  it("builds a simple INSERT", () => {
    const q = QueryBuilder.insert("users").set("name", "Alice").set("age", 30).build();
    expect(q.sql).toBe('INSERT INTO "users" ("name", "age") VALUES ($1, $2)');
    expect(q.params).toEqual(["Alice", 30]);
  });

  it("builds INSERT with values()", () => {
    const q = QueryBuilder.insert("users").values({ name: "Bob", age: 25 }).build();
    expect(q.sql).toBe('INSERT INTO "users" ("name", "age") VALUES ($1, $2)');
    expect(q.params).toEqual(["Bob", 25]);
  });

  it("builds INSERT with RETURNING", () => {
    const q = QueryBuilder.insert("users").set("name", "Alice").returning("id").build();
    expect(q.sql).toBe('INSERT INTO "users" ("name") VALUES ($1) RETURNING "id"');
  ...
  it("builds INSERT with entity class", () => {
    const q = QueryBuilder.insert(User)
      ...
      .returning("id")
      .build();
    expect(q.sql).toBe('INSERT INTO "users" ("user_name", "email_address", "age") VALUES ($1, $2, $3) RETURNING "id"');
    expect(q.params).toEqual(["Charlie", "c@test.com", 28]);
  });

  it("builds INSERT with multiple RETURNING columns", () => {
    const q = QueryBuilder.insert("users").set("name", "Alice").returning("id", "created_at").build();
    expect(q.sql).toBe('INSERT INTO "users" ("name") VALUES ($1) RETURNING "id", "created_at"');
  });
});

describe("QueryBuilder.update", () => {
  it("builds a simple UPDATE", () => {
    const q = QueryBuilder.update("users").set("name", "Alice").where(col("id").eq(1)).build();
    expect(q.sql).toBe('UPDATE "users" SET "name" = $1 WHERE "id" = $2');
    expect(q.params).toEqual(["Alice", 1]);
  });

  it("builds UPDATE with multiple SET", () => {
    const q = QueryBuilder.update("users").set("name", "Alice").set("age", 31).where(col("id").eq(1)).build();
    expect(q.sql).toBe('UPDATE "users" SET "name" = $1, "age" = $2 WHERE "id" = $3');
    expect(q.params).toEqual(["Alice", 31, 1]);
  });

  it("builds UPDATE with values()", () => {
    const q = QueryBuilder.update("users").values({ name: "Alice", age: 31 }).where(col("id").eq(1)).build();
    expect(q.sql).toBe('UPDATE "users" SET "name" = $1, "age" = $2 WHERE "id" = $3');
    expect(q.params).toEqual(["Alice", 31, 1]);
  });
  ...
  it("builds UPDATE with AND in WHERE", () => {
    const q = QueryBuilder.update("users")
      ...
      .build();
    expect(q.sql).toBe('UPDATE "users" SET "active" = $1 WHERE ("age" < $2 AND "verified" = $3)');
    expect(q.params).toEqual([false, 18, false]);
  });

  it("builds UPDATE with RETURNING", () => {
    const q = QueryBuilder.update("users").set("name", "Alice").where(col("id").eq(1)).returning("*").build();
    expect(q.sql).toBe('UPDATE "users" SET "name" = $1 WHERE "id" = $2 RETURNING *');
  });
  ...
  it("builds UPDATE with entity class", () => {
    const q = QueryBuilder.update(User).set("user_name", "Updated").where(col("id").eq(1)).build();
    expect(q.sql).toBe('UPDATE "users" SET "user_name" = $1 WHERE "id" = $2');
    expect(q.params).toEqual(["Updated", 1]);
  });

  it("and() on empty where sets the criteria", () => {
    const q = QueryBuilder.update("users").set("active", false).and(col("id").eq(1)).build();
...
describe("QueryBuilder.delete", () => {
  it("builds a simple DELETE", () => {
    const q = QueryBuilder.delete("users").where(col("id").eq(1)).build();
    expect(q.sql).toBe('DELETE FROM "users" WHERE "id" = $1');
    expect(q.params).toEqual([1]);
  });

  it("builds DELETE with compound WHERE", () => {
    const q = QueryBuilder.delete("users").where(col("active").eq(false)).and(col("age").lt(18)).build();
    expect(q.sql).toBe('DELETE FROM "users" WHERE ("active" = $1 AND "age" < $2)');
    expect(q.params).toEqual([false, 18]);
  });

  it("builds DELETE with RETURNING", () => {
    const q = QueryBuilder.delete("users").where(col("id").eq(1)).returning("id", "name").build();
    expect(q.sql).toBe('DELETE FROM "users" WHERE "id" = $1 RETURNING "id", "name"');
  });
  ...
  it("builds DELETE with entity class", () => {
    const q = QueryBuilder.delete(User).where(col("id").eq(42)).build();
    expect(q.sql).toBe('DELETE FROM "users" WHERE "id" = $1');
    expect(q.params).toEqual([42]);
  });

  it("and() on empty where sets the criteria", () => {
    const q = QueryBuilder.delete("users").and(col("id").eq(1)).build();
    expect(q.sql).toBe('DELETE FROM "users" WHERE "id" = $1');
    expect(q.params).toEqual([1]);
  });
});
