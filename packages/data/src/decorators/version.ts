...
export function Version<T>(_target: undefined, context: ClassFieldDecoratorContext<T>): void {
  context.addInitializer(function (this: T) {
    ...
    if (existing !== undefined && existing !== context.name) {
      throw new Error(
        `Multiple @Version fields found on ${constructor.name}. ` + `Only one @Version field is allowed per entity.`,
      );
    }
...
export function getVersionField(target: object): string | symbol | undefined {
