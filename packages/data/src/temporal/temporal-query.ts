...
export class TemporalQueryBuilder {
  ...
  findAsOf(timestamp: Date | string): { sql: string; params: SqlValue[] } {
    ...
    const sql =
      `SELECT * FROM ${table} WHERE ${validFrom} <= $1 AND (${validTo} > $1 OR ${validTo} IS NULL)`;
  ...
  findHistory(startDate: Date | string, endDate: Date | string): { sql: string; params: SqlValue[] } {
    ...
    const sql =
      `SELECT * FROM ${table} WHERE ${validFrom} >= $1 AND ${validFrom} <= $2 ORDER BY ${validFrom}`;
  ...
  findHistoryById(
    ...
    const sql =
      `SELECT * FROM ${table} WHERE "id" = $1 AND ${validFrom} >= $2 AND ${validFrom} <= $3 ORDER BY ${validFrom}`;
