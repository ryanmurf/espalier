...
function createMockConnection(opts?: {
  ...
  return {
    ...
    prepareNamedStatement: vi.fn(),
    prepareBatchStatement: vi.fn(),
...
describe("MysqlMigrationRunner", () => {
  ...
  describe("run()", () => {
    it("applies pending migrations in version order", async () => {
      ...
      const getAppliedConn = createMockConnection({
        preparedStatements: [appliedPs],
      });

      // Second call: apply migrations
      // MySQL pattern: DDL outside tx, then tx for INSERT into tracking table
    ...
    it("rolls back tracking INSERT on failure (MySQL DDL is non-transactional)", async () => {
      ...
      const stmt = createMockStatement();
      const insertPs = createMockPreparedStatement();
      insertPs.executeUpdate.mockRejectedValueOnce(new Error("insert failed"));
      const applyConn = createMockConnection({
        transactions: [tx],
        statements: [stmt],
        preparedStatements: [insertPs],
