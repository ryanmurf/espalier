...
describe("MysqlNamedPreparedStatement", () => {
  describe("executeQuery()", () => {
    it("converts named params and executes with ? placeholders", async () => {
      ...
      const stmt = new MysqlNamedPreparedStatement(mockConn, "SELECT * FROM users WHERE name = :name AND age > :age");
      stmt.setNamedParameter("name", "Alice");
      stmt.setNamedParameter("age", 18);
      const rs = await stmt.executeQuery();

      // Named params -> $1/$2 -> ? for mysql2
      expect(mockConn.execute).toHaveBeenCalledWith("SELECT * FROM users WHERE name = ? AND age > ?", ["Alice", 18]);
    ...
    it("reuses param index for duplicate named params", async () => {
      ...
      const stmt = new MysqlNamedPreparedStatement(mockConn, "SELECT * FROM t WHERE a = :val OR b = :val");
      ...
      const call = (mockConn.execute as ReturnType<typeof vi.fn>).mock.calls[0];
      expect(call[1]).toEqual([42]);
    });

    it("wraps mysql2 errors in QueryError", async () => {
      const mockConn = createMockConnection();
      (mockConn.execute as ReturnType<typeof vi.fn>).mockRejectedValue(new Error("table not found"));

      const stmt = new MysqlNamedPreparedStatement(mockConn, "SELECT * FROM missing WHERE id = :id");
  ...
  describe("executeUpdate()", () => {
    it("executes update with named params and returns row count", async () => {
      ...
      const stmt = new MysqlNamedPreparedStatement(mockConn, "UPDATE users SET name = :name WHERE id = :id");
      stmt.setNamedParameter("name", "Bob");
      stmt.setNamedParameter("id", 5);
      const count = await stmt.executeUpdate();

      expect(mockConn.execute).toHaveBeenCalledWith("UPDATE users SET name = ? WHERE id = ?", ["Bob", 5]);
      expect(count).toBe(1);
    });
    ...
    it("returns 0 when affectedRows is undefined", async () => {
      ...
      const stmt = new MysqlNamedPreparedStatement(mockConn, "UPDATE t SET x = :x");
    ...
    it("wraps mysql2 errors in QueryError", async () => {
      const mockConn = createMockConnection();
      (mockConn.execute as ReturnType<typeof vi.fn>).mockRejectedValue(new Error("constraint violation"));

      const stmt = new MysqlNamedPreparedStatement(mockConn, "INSERT INTO t (a) VALUES (:a)");
  ...
  describe("parameter handling", () => {
    it("returns null for unset named params", async () => {
      ...
      const stmt = new MysqlNamedPreparedStatement(mockConn, "SELECT * FROM t WHERE a = :a AND b = :b");
