...
describe.skipIf(!canConnect)("E2E: MySQL migrations", { timeout: 15000 }, () => {
  let ds: MysqlDataSource;
  let runner: MysqlMigrationRunner;

  const migrations: Migration[] = [
    createMigration(
      "001",
      "Create e2e_mig_users table",
      "CREATE TABLE e2e_mig_users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255) NOT NULL)",
      "DROP TABLE IF EXISTS e2e_mig_users",
    ),
    createMigration(
      "002",
      "Add email column",
      "ALTER TABLE e2e_mig_users ADD COLUMN email VARCHAR(255)",
  ...
  beforeAll(async () => {
    ...
    runner = new MysqlMigrationRunner(ds, {
  ...
  it("initializes migration tracking table", async () => {
    ...
    const rs = await stmt.executeQuery(
      `SELECT 1 FROM information_schema.tables WHERE table_name = '${MIGRATION_TABLE}'`,
    );
  ...
  it("runs all pending migrations", async () => {
    ...
    // Verify the table was created and the column was added
    const conn = await ds.getConnection();
    const stmt = conn.createStatement();
  ...
  it("does nothing when run again with same migrations", async () => {
    // Should not throw and should not re-apply
    await runner.run(migrations);
    const applied = await runner.getAppliedMigrations();
    expect(applied).toHaveLength(2);
  });
