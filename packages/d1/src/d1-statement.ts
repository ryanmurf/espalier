...
/**
 * Convert $1, $2 positional params to D1's ? placeholder style.
 * Returns the converted SQL and a mapping of original indices.
 */
...
export class D1StatementImpl implements Statement {
  constructor(protected readonly db: D1Database) {}

  async executeQuery(sql: string): Promise<ResultSet> {
    const logger = getGlobalLogger().child("d1-query");
  ...
  async close(): Promise<void> {
    // Statement does not own the D1 binding lifecycle
  }
}

export class D1PreparedStatementImpl extends D1StatementImpl implements PreparedStatement {
