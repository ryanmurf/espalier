  ...
  () => {
    ...
    it("commit persists inserted data", async () => {
      ...
      const rs = await stmt.executeQuery(
        `SELECT * FROM ${TABLE} WHERE name = 'TxAlice'`,
      );
    ...
    it("rollback discards inserted data", async () => {
      ...
      const stmt = conn.createStatement();
      const rs = await stmt.executeQuery(
        `SELECT * FROM ${TABLE} WHERE name = 'TxBob'`,
      );
    ...
    describe("connection lifecycle", () => {
      ...
      it("operations throw after connection.close()", async () => {
        ...
        expect(() => conn2.prepareStatement("SELECT 1")).toThrow(
          "Connection is closed",
        );
        await expect(conn2.beginTransaction()).rejects.toThrow(
          "Connection is closed",
        );
      });
    });
