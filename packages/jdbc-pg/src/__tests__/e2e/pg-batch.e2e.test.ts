...
describe.skipIf(!canConnect)(
  ...
  () => {
    ...
    it("batch inserts 100+ rows and verifies count", async () => {
      const batch = conn.prepareBatchStatement(
    ...
    it("batch update modifies multiple rows individually", async () => {
      ...
      const insert = conn.prepareBatchStatement(
        `INSERT INTO ${TABLE} (name, age) VALUES ($1, $2)`,
      );
      ...
      // Batch update
      const update = conn.prepareBatchStatement(
      ...
      // Verify updates
      const stmt = conn.createStatement();
      const rsAlice = await stmt.executeQuery(
        `SELECT age FROM ${TABLE} WHERE name = 'Alice'`,
      );
      await rsAlice.next();
      expect(rsAlice.getNumber("age")).toBe(31);

      const rsBob = await stmt.executeQuery(
        `SELECT age FROM ${TABLE} WHERE name = 'Bob'`,
      );
      await rsBob.next();
    ...
    it("batch delete removes multiple rows individually", async () => {
      ...
      // Batch delete
      const del = conn.prepareBatchStatement(
    ...
    it("batch insert with NULL values", async () => {
      ...
      const rs1 = await stmt.executeQuery(
        `SELECT email FROM ${TABLE} WHERE name = 'NullEmail'`,
      );
      await rs1.next();
      expect(rs1.getString("email")).toBeNull();

      const rs2 = await stmt.executeQuery(
        `SELECT age FROM ${TABLE} WHERE name = 'NullAge'`,
      );
      await rs2.next();
    ...
    it("batch insert within a transaction - commit", async () => {
      ...
      const stmt = conn.createStatement();
      const rs = await stmt.executeQuery(
        `SELECT COUNT(*) AS cnt FROM ${TABLE} WHERE name LIKE 'TxCommit_%'`,
      );
      await rs.next();
    ...
    it("batch insert within a transaction - rollback", async () => {
      ...
      const stmt = conn.createStatement();
      const rs = await stmt.executeQuery(
        `SELECT COUNT(*) AS cnt FROM ${TABLE} WHERE name LIKE 'TxRollback_%'`,
      );
      await rs.next();
    ...
    it("batch insert with constraint violation throws QueryError", async () => {
      ...
      await stmt.executeUpdate(
        `DROP INDEX IF EXISTS idx_batch_unique_name`,
      );
    });
    ...
    it("batch insert with mixed data types", async () => {
      ...
      const stmt = conn.createStatement();
      const rs = await stmt.executeQuery(
        `SELECT * FROM ${TABLE} WHERE name LIKE 'TypeTest%' ORDER BY name`,
      );
