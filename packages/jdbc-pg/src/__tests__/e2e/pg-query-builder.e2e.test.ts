import { and, col, not, or, QueryBuilder } from "espalier-data";
...
describe.skipIf(!canConnect)("E2E: QueryBuilder against Postgres", { timeout: 15000 }, () => {
  let ds: PgDataSource;
  let conn: Connection;
  ...
  async function reseed() {
    ...
    for (const r of SEED_ROWS) {
      const ps = conn.prepareStatement(`INSERT INTO ${TABLE} (name, email, age, active) VALUES ($1, $2, $3, $4)`);
  ...
  describe("SelectBuilder", () => {
    ...
    it("selects specific columns", async () => {
      const query = QueryBuilder.select(TABLE).columns("name", "age").build();
      const rows = await collectRows(query);
      expect(rows.length).toBe(5);
    ...
    it("where eq criteria", async () => {
      const query = QueryBuilder.select(TABLE).where(col("name").eq("Alice")).build();
      const rows = await collectRows(query);
    ...
    it("where neq criteria", async () => {
      const query = QueryBuilder.select(TABLE).where(col("name").neq("Alice")).build();
      const rows = await collectRows(query);
    ...
    it("where gt criteria", async () => {
      const query = QueryBuilder.select(TABLE).where(col("age").gt(30)).build();
      const rows = await collectRows(query);
      expect(rows.length).toBe(1);
    ...
    it("where gte criteria", async () => {
      const query = QueryBuilder.select(TABLE).where(col("age").gte(30)).build();
      const rows = await collectRows(query);
    ...
    it("where lt criteria", async () => {
      const query = QueryBuilder.select(TABLE).where(col("age").lt(25)).build();
      const rows = await collectRows(query);
      expect(rows.length).toBe(1);
    ...
    it("where lte criteria", async () => {
      const query = QueryBuilder.select(TABLE).where(col("age").lte(25)).build();
      const rows = await collectRows(query);
      expect(rows.length).toBe(2);
      const names = rows.map((r) => r.name).sort();
    ...
    it("where like criteria", async () => {
      const query = QueryBuilder.select(TABLE).where(col("email").like("%@example.com")).build();
      const rows = await collectRows(query);
      expect(rows.length).toBe(4);
    });
    ...
    it("where between criteria", async () => {
      const query = QueryBuilder.select(TABLE).where(col("age").between(25, 30)).build();
      const rows = await collectRows(query);
      expect(rows.length).toBe(3);
      const names = rows.map((r) => r.name).sort();
    ...
    it("where isNull criteria", async () => {
      const query = QueryBuilder.select(TABLE).where(col("email").isNull()).build();
      const rows = await collectRows(query);
      expect(rows.length).toBe(1);
      expect(rows[0].name).toBe("Eve");
    });
    ...
    it("where isNotNull criteria", async () => {
      const query = QueryBuilder.select(TABLE).where(col("email").isNotNull()).build();
      const rows = await collectRows(query);
      expect(rows.length).toBe(4);
    });

    it("AND composition via chaining", async () => {
      const query = QueryBuilder.select(TABLE).where(col("age").gte(25)).and(col("active").eq(true)).build();
      const rows = await collectRows(query);
      expect(rows.length).toBe(3);
      const names = rows.map((r) => r.name).sort();
      expect(names).toEqual(["Alice", "Bob", "Diana"]);
    ...
    it("OR composition via chaining", async () => {
      const query = QueryBuilder.select(TABLE).where(col("name").eq("Alice")).or(col("name").eq("Eve")).build();
      const rows = await collectRows(query);
      expect(rows.length).toBe(2);
      const names = rows.map((r) => r.name).sort();
    ...
    it("complex AND/OR composition with helper functions", async () => {
      ...
      const criteria = or(and(col("age").gt(28), col("active").eq(true)), col("name").eq("Eve"));
    ...
    it("ORDER BY ascending", async () => {
      const query = QueryBuilder.select(TABLE).columns("name", "age").orderBy("age", "ASC").build();
      const rows = await collectRows(query);
    ...
    it("ORDER BY descending", async () => {
      const query = QueryBuilder.select(TABLE).columns("name", "age").orderBy("age", "DESC").build();
      const rows = await collectRows(query);
      const ages = rows.map((r) => r.age);
    ...
    it("LIMIT", async () => {
      const query = QueryBuilder.select(TABLE).orderBy("age", "ASC").limit(3).build();
      const rows = await collectRows(query);
      expect(rows.length).toBe(3);
      const ages = rows.map((r) => r.age);
    ...
    it("OFFSET", async () => {
      const query = QueryBuilder.select(TABLE).orderBy("age", "ASC").limit(2).offset(2).build();
      const rows = await collectRows(query);
      expect(rows.length).toBe(2);
      const ages = rows.map((r) => r.age);
  ...
  describe("InsertBuilder", () => {
    it("inserts a row using set()", async () => {
      ...
      // Verify
      const check = QueryBuilder.select(TABLE).where(col("name").eq("Frank")).build();
    ...
    it("inserts a row using values()", async () => {
      const query = QueryBuilder.insert(TABLE).values({ name: "Grace", email: "grace@example.com", age: 33 }).build();
      const count = await executeUpdate(query);
      expect(count).toBe(1);
    });
  ...
  describe("UpdateBuilder", () => {
    it("updates rows matching criteria", async () => {
      const query = QueryBuilder.update(TABLE).set("age", 31).where(col("name").eq("Alice")).build();
      const count = await executeUpdate(query);
      expect(count).toBe(1);

      // Verify
      const check = QueryBuilder.select(TABLE).columns("age").where(col("name").eq("Alice")).build();
      const rows = await collectRows(check);
      expect(rows[0].age).toBe(31);
    });

    it("updates with values()", async () => {
      const query = QueryBuilder.update(TABLE).values({ age: 26, active: false }).where(col("name").eq("Bob")).build();
      const count = await executeUpdate(query);
      expect(count).toBe(1);

      const check = QueryBuilder.select(TABLE).columns("age", "active").where(col("name").eq("Bob")).build();
      const rows = await collectRows(check);
    ...
    it("returns 0 when no rows match", async () => {
      const query = QueryBuilder.update(TABLE).set("age", 100).where(col("name").eq("Nobody")).build();
      const count = await executeUpdate(query);
      expect(count).toBe(0);
    });
  });
  ...
  describe("DeleteBuilder", () => {
    it("deletes rows matching criteria", async () => {
      // Insert a temp row
      const insertQuery = QueryBuilder.insert(TABLE).set("name", "ToDeleteViaQB").set("age", 99).build();
      await executeUpdate(insertQuery);

      const deleteQuery = QueryBuilder.delete(TABLE).where(col("name").eq("ToDeleteViaQB")).build();
      const count = await executeUpdate(deleteQuery);
      expect(count).toBe(1);

      // Verify deleted
      const check = QueryBuilder.select(TABLE).where(col("name").eq("ToDeleteViaQB")).build();
      const rows = await collectRows(check);
      expect(rows.length).toBe(0);
    });

    it("deletes with RETURNING clause", async () => {
      const insertQuery = QueryBuilder.insert(TABLE).set("name", "ReturnDelete").set("age", 88).build();
      await executeUpdate(insertQuery);
    ...
    it("deletes with AND criteria", async () => {
      const insertQuery = QueryBuilder.insert(TABLE).values({ name: "MultiCritDel", age: 77, active: false }).build();
      await executeUpdate(insertQuery);
    ...
    it("returns 0 when no rows match", async () => {
      const query = QueryBuilder.delete(TABLE).where(col("name").eq("Nobody")).build();
      const count = await executeUpdate(query);
      expect(count).toBe(0);
    });
  });
