...
describe.skipIf(!canConnect)("E2E: Data type handling", { timeout: 10000 }, () => {
  let ds: PgDataSource;
  let conn: Connection;
  ...
  it("handles NULL values for all nullable columns", async () => {
    ...
    const stmt = conn.createStatement();
    const rs = await stmt.executeQuery(`SELECT * FROM ${TABLE} ORDER BY id DESC LIMIT 1`);
  ...
  it("handles boolean true and false", async () => {
    const psTrue = conn.prepareStatement(`INSERT INTO ${TABLE} (text_col, bool_col) VALUES ($1, $2)`);
    psTrue.setParameter(1, "bool_true");
    psTrue.setParameter(2, true);
    await psTrue.executeUpdate();

    const psFalse = conn.prepareStatement(`INSERT INTO ${TABLE} (text_col, bool_col) VALUES ($1, $2)`);
    psFalse.setParameter(1, "bool_false");
    psFalse.setParameter(2, false);
    await psFalse.executeUpdate();

    const stmt = conn.createStatement();
    const rsTrue = await stmt.executeQuery(`SELECT bool_col FROM ${TABLE} WHERE text_col = 'bool_true'`);
    expect(await rsTrue.next()).toBe(true);
    expect(rsTrue.getBoolean("bool_col")).toBe(true);

    const rsFalse = await stmt.executeQuery(`SELECT bool_col FROM ${TABLE} WHERE text_col = 'bool_false'`);
    expect(await rsFalse.next()).toBe(true);
    expect(rsFalse.getBoolean("bool_col")).toBe(false);
  });

  it("handles integer boundary values", async () => {
    const ps = conn.prepareStatement(`INSERT INTO ${TABLE} (text_col, int_col) VALUES ($1, $2)`);
    ps.setParameter(1, "max_int");
    ps.setParameter(2, 2147483647);
    await ps.executeUpdate();

    const ps2 = conn.prepareStatement(`INSERT INTO ${TABLE} (text_col, int_col) VALUES ($1, $2)`);
    ps2.setParameter(1, "min_int");
    ps2.setParameter(2, -2147483648);
    await ps2.executeUpdate();

    const stmt = conn.createStatement();
    const rsMax = await stmt.executeQuery(`SELECT int_col FROM ${TABLE} WHERE text_col = 'max_int'`);
    expect(await rsMax.next()).toBe(true);
    expect(rsMax.getNumber("int_col")).toBe(2147483647);

    const rsMin = await stmt.executeQuery(`SELECT int_col FROM ${TABLE} WHERE text_col = 'min_int'`);
    expect(await rsMin.next()).toBe(true);
    expect(rsMin.getNumber("int_col")).toBe(-2147483648);
  });

  it("handles floating point values", async () => {
    const ps = conn.prepareStatement(`INSERT INTO ${TABLE} (text_col, float_col) VALUES ($1, $2)`);
    ps.setParameter(1, "float_test");
    ps.setParameter(2, Math.PI);
    await ps.executeUpdate();

    const stmt = conn.createStatement();
    const rs = await stmt.executeQuery(`SELECT float_col FROM ${TABLE} WHERE text_col = 'float_test'`);
    expect(await rs.next()).toBe(true);
    expect(rs.getNumber("float_col")).toBeCloseTo(Math.PI, 10);
  });

  it("handles timestamptz values", async () => {
    const now = new Date();
    const ps = conn.prepareStatement(`INSERT INTO ${TABLE} (text_col, ts_col) VALUES ($1, $2)`);
    ps.setParameter(1, "ts_test");
    ps.setParameter(2, now);
    await ps.executeUpdate();

    const stmt = conn.createStatement();
    const rs = await stmt.executeQuery(`SELECT ts_col FROM ${TABLE} WHERE text_col = 'ts_test'`);
    expect(await rs.next()).toBe(true);
    ...
    expect(Math.abs(retrieved!.getTime() - now.getTime())).toBeLessThan(1000);
  });

  it("handles Uint8Array (bytea) values", async () => {
    const bytes = new Uint8Array([0x00, 0x01, 0xff, 0xfe, 0x42]);
    const ps = conn.prepareStatement(`INSERT INTO ${TABLE} (text_col, bytea_col) VALUES ($1, $2)`);
    ps.setParameter(1, "bytea_test");
    ps.setParameter(2, bytes);
    await ps.executeUpdate();

    const stmt = conn.createStatement();
    const rs = await stmt.executeQuery(`SELECT bytea_col FROM ${TABLE} WHERE text_col = 'bytea_test'`);
    expect(await rs.next()).toBe(true);
    const row = rs.getRow();
  ...
  it("handles special characters in text", async () => {
    ...
    const selectPs = conn.prepareStatement(`SELECT text_col FROM ${TABLE} WHERE text_col = $1`);
  ...
  it("handles empty string vs NULL", async () => {
    const ps = conn.prepareStatement(`INSERT INTO ${TABLE} (text_col, nullable_col) VALUES ($1, $2)`);
    ps.setParameter(1, "empty_vs_null");
    ps.setParameter(2, "");
    await ps.executeUpdate();

    const stmt = conn.createStatement();
    const rs = await stmt.executeQuery(`SELECT nullable_col FROM ${TABLE} WHERE text_col = 'empty_vs_null'`);
    expect(await rs.next()).toBe(true);
