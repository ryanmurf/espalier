...
describe.skipIf(!canConnect)("E2E: Named parameter queries", { timeout: 10000 }, () => {
  let ds: PgDataSource;
  ...
  it("inserts a row with named parameters", async () => {
    const ps = conn.prepareNamedStatement(`INSERT INTO ${TABLE} (name, email, age) VALUES (:name, :email, :age)`);
  ...
  it("selects with named parameter WHERE clause", async () => {
    // Insert first
    const insert = conn.prepareNamedStatement(`INSERT INTO ${TABLE} (name, email, age) VALUES (:name, :email, :age)`);
    ...
    const ps = conn.prepareNamedStatement(`SELECT * FROM ${TABLE} WHERE name = :name`);
  ...
  it("updates with named parameters", async () => {
    const insert = conn.prepareNamedStatement(`INSERT INTO ${TABLE} (name, age) VALUES (:name, :age)`);
    insert.setNamedParameter("name", "Charlie");
    insert.setNamedParameter("age", 35);
    await insert.executeUpdate();

    const update = conn.prepareNamedStatement(`UPDATE ${TABLE} SET age = :newAge WHERE name = :name`);
    ...
    // Verify
    const stmt = conn.createStatement();
    const rs = await stmt.executeQuery(`SELECT age FROM ${TABLE} WHERE name = 'Charlie'`);
    await rs.next();
  ...
  it("deletes with named parameters", async () => {
    ...
    const del = conn.prepareNamedStatement(`DELETE FROM ${TABLE} WHERE name = :name`);
    del.setNamedParameter("name", "ToDelete");
    const count = await del.executeUpdate();
    expect(count).toBe(1);

    const stmt = conn.createStatement();
    const rs = await stmt.executeQuery(`SELECT COUNT(*) AS cnt FROM ${TABLE} WHERE name = 'ToDelete'`);
    await rs.next();
  ...
  it("handles multiple different named parameters", async () => {
    ...
    const select = conn.prepareNamedStatement(`SELECT * FROM ${TABLE} WHERE name = :name AND age = :age`);
  ...
  it("handles reused named parameter (same name appears twice)", async () => {
    ...
    const ps = conn.prepareNamedStatement(`SELECT * FROM ${TABLE} WHERE age >= :minAge AND age <= :minAge + 5`);
  ...
  it("handles NULL named parameter values", async () => {
    ...
    const select = conn.prepareNamedStatement(`SELECT email, age FROM ${TABLE} WHERE name = :name`);
  ...
  it("named params work within a transaction", async () => {
    ...
    const ps = conn.prepareNamedStatement(`INSERT INTO ${TABLE} (name, age) VALUES (:name, :age)`);
    ...
    const stmt = conn.createStatement();
    const rs = await stmt.executeQuery(`SELECT COUNT(*) AS cnt FROM ${TABLE} WHERE name = 'TxNamed'`);
    await rs.next();
    expect(rs.getNumber("cnt")).toBe(0);
  });
  ...
  it("throws QueryError for invalid named param SQL", async () => {
    const ps = conn.prepareNamedStatement(`SELEKT * FROM ${TABLE} WHERE name = :name`);
  ...
  it("missing named parameter defaults to null", async () => {
    const ps = conn.prepareNamedStatement(`INSERT INTO ${TABLE} (name, email) VALUES (:name, :email)`);
    ps.setNamedParameter("name", "MissingParam");
    // Intentionally not setting :email
    await ps.executeUpdate();

    const stmt = conn.createStatement();
    const rs = await stmt.executeQuery(`SELECT email FROM ${TABLE} WHERE name = 'MissingParam'`);
    await rs.next();
    expect(rs.getString("email")).toBeNull();
  });
  ...
  it("prepareNamedStatement throws when connection is closed", async () => {
    ...
    await conn2.close();
    expect(() => conn2.prepareNamedStatement(`SELECT * FROM ${TABLE} WHERE name = :name`)).toThrow(
