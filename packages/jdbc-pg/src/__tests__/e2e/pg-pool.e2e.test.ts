...
describe.skipIf(!canConnect)("E2E: Connection pool behavior", { timeout: 15000 }, () => {
  let ds: PgDataSource;
  ...
  it("connections are independent - work on one does not affect another", async () => {
    ...
    const ps1 = conn1.prepareStatement(`INSERT INTO ${TABLE} (name, email, age) VALUES ($1, $2, $3)`);
    ps1.setParameter(1, "PoolUser1");
    ps1.setParameter(2, "pool1@example.com");
    ps1.setParameter(3, 10);
    await ps1.executeUpdate();

    const ps2 = conn2.prepareStatement(`INSERT INTO ${TABLE} (name, email, age) VALUES ($1, $2, $3)`);
    ...
    const rs = await stmt.executeQuery(`SELECT COUNT(*) AS cnt FROM ${TABLE} WHERE name IN ('PoolUser1', 'PoolUser2')`);
    await rs.next();
  ...
  it("transaction on one connection does not affect another", async () => {
    ...
    const ps = conn1.prepareStatement(`INSERT INTO ${TABLE} (name, email, age) VALUES ($1, $2, $3)`);
    ...
    const stmt2 = conn2.createStatement();
    const rs2 = await stmt2.executeQuery(`SELECT COUNT(*) AS cnt FROM ${TABLE} WHERE name = 'TxIsolated'`);
    await rs2.next();
