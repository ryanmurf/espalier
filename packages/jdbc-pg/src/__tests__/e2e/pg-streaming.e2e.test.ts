...
import { PgStatement } from "../../pg-statement.js";
import type { PgDataSource } from "../../pg-data-source.js";
import type { StreamingResultSet } from "espalier-jdbc";
import {
...
describe.skipIf(!canConnect)(
  "E2E: Streaming/cursor-based ResultSet",
  { timeout: 30000 },
  () => {
    let ds: PgDataSource;
    ...
    it("reads data correctly via getString and getNumber", async () => {
      const stmt = getStreamingStatement();
      const rs = await stmt.executeStreamingQuery(
        `SELECT name, value FROM ${TABLE} ORDER BY id LIMIT 5`,
      );
      rs.setCursorSize(2);
    ...
    it("getRow returns current row object", async () => {
      const stmt = getStreamingStatement();
      const rs = await stmt.executeStreamingQuery(
        `SELECT name, value FROM ${TABLE} ORDER BY id LIMIT 1`,
      );
    ...
    it("getDate reads timestamptz correctly", async () => {
      const stmt = getStreamingStatement();
      const rs = await stmt.executeStreamingQuery(
        `SELECT created_at FROM ${TABLE} ORDER BY id LIMIT 1`,
      );
    ...
    it("getString by column index works", async () => {
      const stmt = getStreamingStatement();
      const rs = await stmt.executeStreamingQuery(
        `SELECT name FROM ${TABLE} ORDER BY id LIMIT 1`,
      );
    ...
    it("getMetadata returns column info from buffered row", async () => {
      const stmt = getStreamingStatement();
      const rs = await stmt.executeStreamingQuery(
        `SELECT name, value FROM ${TABLE} LIMIT 1`,
      );
    ...
    it("async iteration over streaming result set", async () => {
      const stmt = getStreamingStatement();
      const rs = await stmt.executeStreamingQuery(
        `SELECT name, value FROM ${TABLE} ORDER BY id LIMIT 50`,
      );
      rs.setCursorSize(10);
    ...
    it("streaming with cursor size 1 (row-by-row)", async () => {
      const stmt = getStreamingStatement();
      const rs = await stmt.executeStreamingQuery(
        `SELECT value FROM ${TABLE} ORDER BY id LIMIT 10`,
      );
      rs.setCursorSize(1);
    ...
    it("streaming with large cursor size (larger than result)", async () => {
      const stmt = getStreamingStatement();
      const rs = await stmt.executeStreamingQuery(
        `SELECT value FROM ${TABLE} ORDER BY id LIMIT 5`,
      );
